...
def test_oidc_logout_redirects_to_end_session(  # type: ignore[no-untyped-def]
    ...
    assert "id_token_hint=" in logout.headers["location"]
...
def test_session_is_expired_handles_missing_field() -> None:
    assert oidc_module.session_is_expired({}) is False
    assert oidc_module.session_is_expired({"expires_at": int(time.time()) - 60}) is True
    assert oidc_module.session_is_expired({"expires_at": int(time.time()) + 3600}) is False


@respx.mock
def test_oidc_refresh_silently_renews_session(  # type: ignore[no-untyped-def]
    monkeypatch: pytest.MonkeyPatch,
    signing_key: RSAKey,
    jwks: dict[str, Any],
) -> None:
    """An expired session is refreshed transparently before the dependency runs."""
    ...
    with TestClient(application, follow_redirects=False) as tc:
        ...
        # Initial sign-in with a token already on the verge of expiring.
        respx.post(f"{ISSUER}/protocol/openid-connect/token").mock(
            return_value=httpx.Response(
                ...
                json={
                    "access_token": "at-1",
                    "refresh_token": "rt-1",
                    "id_token": _make_id_token(signing_key, expires_in=10),
                    "expires_in": 1,
                    "token_type": "Bearer",
                },
            )
        ...
        # On the next protected hit, expires_at is in the past so the
        # dependency should call the token endpoint again with grant_type=refresh.
        time.sleep(1.1)

        new_token = _make_id_token(signing_key, expires_in=3600)
        respx.post(f"{ISSUER}/protocol/openid-connect/token").mock(
            return_value=httpx.Response(
                200,
                json={
...
@respx.mock
def test_oidc_refresh_failure_clears_session_and_redirects(  # type: ignore[no-untyped-def]
    monkeypatch: pytest.MonkeyPatch,
    signing_key: RSAKey,
    jwks: dict[str, Any],
) -> None:
    ...
    with TestClient(application, follow_redirects=False) as tc:
        ...
        # Provider rejects the refresh — the session must be cleared and the
        # next access redirected to /login.
        time.sleep(1.1)
        respx.post(f"{ISSUER}/protocol/openid-connect/token").mock(
        ...
        assert index.headers["location"].startswith("/login")
